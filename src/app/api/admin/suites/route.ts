...
export async function POST(req: NextRequest) {
  try {
    const { name, description, price, images, capacity, status, roomNumber, category } = await req.json()
    ...
    const suite = await prisma.suite.create({
      data: {
        ...
        isActive: true,
        capacity,
        category: category as SuiteCategory || "REGULAR",
...
export async function PATCH(req: NextRequest) {
  try {
    const { id, name, description, price, images, isActive, capacity, status, roomAssignments } = await req.json()
