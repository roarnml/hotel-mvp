              ...
              <tbody className="divide-y divide-[#75240E]">
                {bookings.length === 0 ? (
                  ...
                  bookings.map((b) => (
                    ...
                    >
                      {/* Basic info */}
                      <td className="px-4 py-3 font-mono text-xs text-[#D55605]">{b.bookingRef}</td>
                      <td className="px-4 py-3">
                      ...
                      {/* Room Assignment */}
                      <td className="px-4 py-3 text-gray-400">{b.roomAssignment?.roomNumber ?? "—"}</td>
                      ...
                      <td className="px-4 py-3 text-xs">
                        {b.details.length === 0
                          ? "—"
                          : b.details.map((d) => (
                              <div key={d.id} className="mb-1 border-b border-gray-700 pb-1">
                                <div>Chalets: {d.chaletCount}</div>
                                <div>Nights: {d.nights}</div>
                                <div>Total: {d.totalAmount} {d.currency}</div>
                                <div>Price/Night: {d.pricePerNight}</div>
                              </div>
                            ))}
                      </td>
                      ...
                      <td className="px-4 py-3 text-xs">
                        {b.payment ? (
                          <div>
                            <div>Provider: {b.payment.provider}</div>
                            <div>Amount: {b.payment.amountPaid ?? b.payment.amount} {b.payment.provider}</div>
                            <div>Status: {b.payment.status}</div>
                          </div>
                        ) : (
                          "—"
                        )}
                      ...
                      <td className="px-4 py-3 text-xs text-gray-500">{b.createdAt}</td>
                    </tr>
                  ))
                )}
              </tbody>
