      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="p-4 bg-neutral-900 rounded-xl border border-[#75240E] animate-pulse h-28"
            ></div>
          ))}
        </div>
      ) : arrivals.length === 0 ? (
            ...
            >
              ...
              <div className="flex justify-between items-center">
                ...
                <div>
                  ...
                  {a.status === "Pending" && !a.eligible && (
                    <span className="px-3 py-1 bg-gray-700/20 text-gray-400 rounded-full text-xs">
                      Not eligible
                    </span>
                  )}
                  {a.status === "Checked-in" && (
...
import { useEffect, useState } from "react"
import { FiUser, FiCheck, FiStar, FiLoader } from "react-icons/fi"
...
export default function StaffArrivalsPage() {
  ...
  // Fetch arrivals with filters
  async function fetchArrivals() {
    setLoading(true)
    try {
      const query = new URLSearchParams()
      if (vipFilter) query.append("vip", vipFilter)
      if (suiteFilter) query.append("suite", suiteFilter)
      if (statusFilter) query.append("status", statusFilter)

      const res = await fetch("/api/staff/arrivals?" + query.toString())
      const data: Arrival[] = await res.json()
  ...
  // Fetch arrivals on mount and whenever filters change
  useEffect(() => {
    fetchArrivals()
  }, [vipFilter, suiteFilter, statusFilter])
...
function ArrivalCard({
  ...
  return (
    <div className="bg-neutral-900 p-4 rounded-xl border border-[#75240E] shadow flex flex-col justify-between hover:bg-[#111111] transition-all duration-300">
      <div className="flex items-center gap-4 mb-4">
        <FiUser className="text-[#75240E] text-xl" />
        <div>
          <div className="font-semibold text-white">{arrival.guestName}</div>
          <div className="text-gray-400 text-sm">
            {arrival.suite} • <span className="font-medium">{arrival.checkIn}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          {arrival.vip ? (
            <span
              className="px-2 py-1 text-[#D55605] bg-[#75240E]/20 rounded-full font-semibold flex items-center gap-1 cursor-pointer"
              onClick={() => onToggleVIP(arrival.id)}
            >
              <FiStar /> VIP
            </span>
          ) : (
